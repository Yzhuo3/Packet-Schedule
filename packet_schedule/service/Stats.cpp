...
void writeDetailedReport(SimulationEngine& engine, const std::string& date)
{
    ...
    {
    case TrafficType::AUDIO: out << "Audio-like";
        break;
    case TrafficType::VIDEO: out << "Video-like";
        break;
    case TrafficType::DATA: out << "Data-like";
        break;
    default: out << "Unknown";
        break;
    }
    ...
    for (auto& entry : engine.nodeStatsMap)
    {
        ...
        double premiumAvg = (stats.premiumCount > 0)
                                ? (stats.premiumDelaySum / stats.premiumCount)
                                : 0.0;
        double assuredAvg = (stats.assuredCount > 0)
                                ? (stats.assuredDelaySum / stats.assuredCount)
                                : 0.0;
        double bestAvg = (stats.bestEffortCount > 0)
                             ? (stats.bestEffortDelaySum / stats.bestEffortCount)
                             : 0.0;
    ...
    for (auto& entry : engine.nodeStatsMap)
    {
        ...
        out << "Node " << nodeId << ":\n"
            << " Premium block number: " << double(stats.droppedPremium) << "\n"
            << " Assured block number: " << double(stats.droppedAssured) << "\n"
            << " Best-effort block number: " << double(stats.droppedBestEffort) << "\n";
    }
    out << "\n";
    ...
    double premiumBlockRatioAll = (totalPremArrivals > 0.0)
                                      ? (totalPremDropped / totalPremArrivals)
                                      : 0.0;
    double assuredBlockRatioAll = (totalAssArrivals > 0.0)
                                      ? (totalAssDropped / totalAssArrivals)
                                      : 0.0;
    double bestBlockRatioAll = (totalBestArrivals > 0.0)
                                   ? (totalBestDropped / totalBestArrivals)
                                   : 0.0;
    ...
    out << "Overall (all nodes) blocking number:\n"
        << " Premium block number: " << totalPremDropped << "\n"
        << " Assured block number: " << totalAssDropped << "\n"
        << " Best-effort block number: " << totalBestDropped << "\n";
    
    out << "Overall (all nodes) total number:\n"
        << " Premium queue total: " << totalPremArrivals << "\n"
        << " Assured queue total: " << totalAssArrivals << "\n"
        << " Best-effort queue total: " << totalBestArrivals << "\n\n";
