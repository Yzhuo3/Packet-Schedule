...
{
    // Initialize an arrival event at time=0 for each traffic source
...
void SimulationEngine::handleArrival(Event* event)
{
    ...
    if (pkt)
    {
        ...
        if (!success)
        {
            ...
            switch (pkt->priority)
            {
            case Priority::PREMIUM: stats.droppedPremium++;
                break;
            case Priority::ASSURED: stats.droppedAssured++;
                break;
            case Priority::BEST_EFFORT: stats.droppedBestEffort++;
                break;
            }
            if (pkt->is_reference)
            {
